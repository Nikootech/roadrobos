...
serve(async (req: Request) => {
  try {
    ...
    if (!newRecord || !newRecord.driver_id) {
      return new Response(JSON.stringify({ error: "Missing driver_id" }), { status: 400 });
    }
    ...
    const body = "A new job has been assigned to you. Tap to view details.";
    const msgData = { type: 'ride_request', id: newRecord.id };

    await sendFcmMessage(profile.fcm_token, title, body, msgData);

    // Log success
    await supabaseClient.from('notification_log').insert({
      user_id: newRecord.driver_id,
