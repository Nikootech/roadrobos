...
class _KycUploadScreenState extends ConsumerState<KycUploadScreen> {
  ...
  Future<String?> _uploadDocument(
      File file, String docType, String userId) async {
    try {
    ...
    try {
      if (_currentStep == 0) {
        if (_aadharFront == null || _aadharBack == null) {
          throw Exception('Aadhaar front and back required');
        }
        final frontUrl =
            await _uploadDocument(_aadharFront!, 'aadhar_front', user.id);
        final backUrl =
            await _uploadDocument(_aadharBack!, 'aadhar_back', user.id);
        if (frontUrl != null) {
        ...
        if (_dlFront == null || _dlBack == null) {
          throw Exception('DL front and back required');
        }
        ...
        if (rcUrl != null) {
          await kycRepo.submitKyc(
              userId: user.id, documentType: 'vehicle_rc', documentUrl: rcUrl);
        }
        ...
        if (selfieUrl != null) {
          await kycRepo.submitKyc(
              userId: user.id, documentType: 'selfie', documentUrl: selfieUrl);
        }
      }
    } catch (e) {
      stepSuccess = false;
