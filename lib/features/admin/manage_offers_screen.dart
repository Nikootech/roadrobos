  ...
  Future<void> _submit() async {
    ...
    try {
      await ref.read(bannerOfferRepositoryProvider).createOffer(
        ...
        image: _imageController.text.trim().isEmpty ? 'assets/banners/b3.jpg' : _imageController.text.trim(),
        cta: _ctaController.text.trim().isEmpty ? 'Claim Now' : _ctaController.text.trim(),
      );
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Offer created!'), backgroundColor: AppColors.successGreen));
        context.pop();
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed: $e'), backgroundColor: AppColors.dangerRed));
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          const Text('Create New Offer', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          const SizedBox(height: 16),
          TextField(controller: _titleController, decoration: const InputDecoration(labelText: 'Title')),
          const SizedBox(height: 12),
          TextField(controller: _subtitleController, decoration: const InputDecoration(labelText: 'Subtitle / Description')),
          const SizedBox(height: 12),
          TextField(controller: _ctaController, decoration: const InputDecoration(labelText: 'Button Text (e.g. Claim Now)')),
          const SizedBox(height: 12),
          TextField(controller: _imageController, decoration: const InputDecoration(labelText: 'Image URL (optional)')),
          ...
          ElevatedButton(
            ...
            child: _isLoading ? const CircularProgressIndicator(color: Colors.white) : const Text('Create Offer'),
          ),
        ],
      ),
    );
