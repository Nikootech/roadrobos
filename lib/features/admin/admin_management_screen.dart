...
class _AdminManagementScreenState extends ConsumerState<AdminManagementScreen> {
  ...
  Future<void> _toggleApproval(String uid, bool currentApproval) async {
    try {
      await ref.read(adminOpsRepositoryProvider).updateEmployeeApproval(uid, !currentApproval);
      if (!mounted) return;
      NavHelpers.showSuccess(
        context,
        currentApproval ? 'Employee access suspended!' : 'Employee approved and activated!',
      );
  ...
  Future<void> _changeRole(String uid, String currentRole) async {
        ...
        content: SizedBox(
          ...
          child: ListView(
            ...
            children: UserRole.values
                .where((role) => role != UserRole.customer && role != UserRole.driver && role != UserRole.admin)
                .map((role) {
              ...
              return ListTile(
                ...
                trailing: isCurrent ? const Icon(Icons.check_circle_rounded, color: AppColors.primaryBlue) : null,
    ...
    if (selectedRole != null && selectedRole != currentRole) {
      try {
        await ref.read(adminOpsRepositoryProvider).updateEmployeeApproval(uid, true, role: selectedRole);
        if (mounted) {
            ...
            if (_isLoading)
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    const Icon(Iconsax.user_remove, size: 48, color: AppColors.textMuted),
                    const SizedBox(height: 12),
                    Text(
                      'No employees found',
                      style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.textSecondary),
                    ),
                    const SizedBox(height: 4),
                    const Text('Registered employees will appear here.', style: TextStyle(color: AppColors.textMuted, fontSize: 13)),
                  ],
                ),
              )
            else
              ListView.separated(
                ...
                itemBuilder: (context, index) {
                  return _buildEmployeeCard(filtered[index]).animate().fadeIn(delay: (index * 50).ms).slideX(begin: 0.05, end: 0);
                },
              ),
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(name, style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.w700)),
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: isApproved ? AppColors.successGreen.withValues(alpha: 0.1) : AppColors.warningAmber.withValues(alpha: 0.1),
                ...
                child: Text(
                  ...
                  style: TextStyle(
                    ...
                    color: isApproved ? AppColors.successGreen : AppColors.warningAmber,
                  ),
                ),
              ),
            ],
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        const Icon(Iconsax.edit_2, size: 12, color: Colors.blueGrey),
                      ],
                    ),
                  ),
                ),
