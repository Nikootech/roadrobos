...
class UserNotifier extends StateNotifier<UserState> {
  ...
  UserNotifier(this._authService, this._userRepository, this._ref)
  ...
  void _init() {
    ...
    _authSubscription =
      ...
      if (sbUser == null) {
        ...
        final hasProfilePic = state.user?.profilePic != null &&
            state.user!.profilePic!.isNotEmpty;
        if (state.user?.id == sbUser.id && !state.isLoading && hasProfilePic) {
          return;
        }
  ...
  void _setupRealtimeListener(String uid) {
    ...
    _profileSubscription =
        _userRepository.getUserStream(uid).listen((updatedUser) async {
      if (updatedUser == null) {
        if (kDebugMode) {
          debugPrint(
              'UserNotifier: Profile deleted from database. Logging out.');
        }
        await logout();
        return;
      }
  ...
  Future<bool> _checkDeviceSession(AppUser user) async {
    ...
    final localDeviceId =
        await _ref.read(localStorageServiceProvider).getLocalDeviceId();
    ...
    if (user.currentDeviceId == null || user.currentDeviceId!.isEmpty) {
      if (kDebugMode) {
        debugPrint(
            'UserNotifier: No device ID registered. Registering $localDeviceId');
      }
      await _userRepository.updateField(
          user.id, 'current_device_id', localDeviceId);
      return true;
    }
    ...
    if (user.currentDeviceId != localDeviceId) {
      if (kDebugMode) {
        debugPrint(
            'UserNotifier: Device mismatch! DB DeviceID: ${user.currentDeviceId}, Local DeviceID: $localDeviceId');
      }

      // Check if we are already logged in and active on this device (meaning we were hijacked)
      if (state.user != null &&
          state.user!.id == user.id &&
          state.user!.currentDeviceId == localDeviceId) {
        if (kDebugMode) {
  ...
  Future<void> confirmSessionTakeover() async {
    ...
    try {
      final localDeviceId =
          await _ref.read(localStorageServiceProvider).getLocalDeviceId();
      await _userRepository.updateField(
          pending.id, 'current_device_id', localDeviceId);
  ...
  Future<void> fetchUserProfile(String uid) async {
    ...
    try {
      ...
      if (user != null) {
        ...
        if (kDebugMode) {
          debugPrint(
              'UserNotifier: getUser returned user=${user.name} (id=${user.id}, role=${user.role})');
        }
        ...
        if (kDebugMode) {
          debugPrint(
              'UserNotifier: Supabase Auth Sync: [OAuth Name: $oauthName, OAuth Pic: $oauthPic]');
        }
        ...
        final changesDetected = updatedName != user.name ||
            updatedPic != user.profilePic ||
            updatedRole != user.role ||
            updatedApproval != user.isApproved;
        final didChangeToDriver =
            updatedRole == UserRole.driver && user.role != UserRole.driver;
        final didChangeToAdmin =
            updatedRole == UserRole.admin && user.role != UserRole.admin;

        if (kDebugMode) {
          debugPrint(
              'UserNotifier: Sync check - needsUpdate=$needsUpdate, changesDetected=$changesDetected, isDemoId=$isDemoId');
        }
        if (needsUpdate && changesDetected && !isDemoId) {
          ...
          if (kDebugMode) {
            debugPrint(
                'UserNotifier: Saving updated profile to database: ${user.toMap()}');
          }
          ...
          if (didChangeToDriver) {
            try {
              ...
              if (kDebugMode) {
                debugPrint(
                    'UserNotifier: Failed to register driver profile: $e');
              }
            }
          }
            ...
            try {
              ...
              final notifications = admins
                  .map((admin) => {
                        ...
                        'message':
                            '${user!.name} has requested to join the Core Team. Awaiting your approval.',
                        'type': 'system',
                      })
                  .toList();
              ...
              if (kDebugMode) {
                debugPrint(
                    'UserNotifier: Failed to send admin notifications: $e');
              }
            }
          }
        ...
        if (!isDemoId) {
          ...
          unawaited(
              _ref.read(notificationServiceProvider).syncTokenToBackend(uid));
        }
      } else {
        if (kDebugMode) {
          debugPrint(
              'UserNotifier: Profile is null. Creating new profile for uid=$uid');
        }
        ...
        if (!isDemoId) {
          if (kDebugMode) {
            debugPrint(
                'UserNotifier: Saving new user profile to database: ${newUser.toMap()}');
          }
            ...
            try {
              ...
              final notifications = admins
                  .map((admin) => {
                        ...
                        'title': 'New Core Team Request',
                        'message':
                            '${newUser.name} has requested to join the Core Team. Awaiting your approval.',
                        'type': 'system',
                      })
                  .toList();
        ...
        state =
            state.copyWith(user: newUser, isLoading: false, isDemo: isDemoId);
  ...
  Future<void> updateProfile(
    ...
    try {
      ...
      debugPrint(
          'Updating Profile: [Name: ${updatedUser.name}, Phone: ${updatedUser.phone}, Email: ${updatedUser.email}]');
  ...
  Future<void> deleteAccountRequest() async {
    ...
    try {
      if (!state.isDemo) {
        await _userRepository.updateField(
            currentAppUser.id, 'deletion_requested', true);
        await _userRepository.updateField(
            currentAppUser.id, 'deletion_requested_at', DateTime.now().utcIso);
      }
      await logout();
    } catch (e) {
      state = state.copyWith(isLoading: false, error: e.toString());
  ...
  Future<void> enable2FA() async {
    ...
    try {
      await _ref
          .read(twoFactorAuthServiceProvider)
          .markMfaEnabledInProfile(uid);
      state = state.copyWith(mfaEnabled: true);
  ...
  Future<void> pickAndUploadProfilePicture(
    ...
    try {
      ...
      final publicUrl = await _userRepository.uploadProfilePicture(
          currentAppUser.id, bytes, extension.isEmpty ? 'jpg' : extension);

      // Update Database
      await _userRepository.updateField(
          currentAppUser.id, 'profile_pic', publicUrl);
