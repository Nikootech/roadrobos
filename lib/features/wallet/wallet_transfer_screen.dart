...
class _WalletTransferScreenState extends ConsumerState<WalletTransferScreen> {
  ...
  void _onPhoneChanged() {
    ...
    if (text.length == 10 && _resolvedRecipient == null && !_isSearching) {
      _lookupRecipient(text);
        ...
        (_resolvedRecipient != null || _searchError != null)) {
      setState(() {
  ...
  Future<void> _lookupRecipient(String phone) async {
    ...
    try {
      final result =
          await ref.read(walletRepositoryProvider).lookupUserByPhone(phone);
      if (!mounted) return;
  ...
  Future<void> _transfer() async {
                ...
                child: Row(
                  children: [
                    CircleAvatar(
                      ...
                      backgroundImage:
                          _resolvedRecipient!['avatar_url'] != null &&
                                  _resolvedRecipient!['avatar_url']
                                      .toString()
                                      .isNotEmpty
                              ? NetworkImage(_resolvedRecipient!['avatar_url'])
                              : null,
                      child: _resolvedRecipient!['avatar_url'] == null ||
                              _resolvedRecipient!['avatar_url']
                                  .toString()
                                  .isEmpty
                          ? Text(
                              _resolvedRecipient!['full_name'] != null &&
                                      _resolvedRecipient!['full_name']
                                          .toString()
                                          .isNotEmpty
                                  ? _resolvedRecipient!['full_name'][0]
                                      .toUpperCase()
                                  : 'U',
    ...
    try {
      final success = await ref
          ...
          .transferFunds(user.id, phone, amount);
      if (success) {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
                ...
                if (_phoneController.text.trim().length >= 10 &&
                  ...
                  Padding(
                    ...
                    child: ElevatedButton(
                      onPressed: () =>
                          _lookupRecipient(_phoneController.text.trim()),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: AppColors.primaryBlue,
                        shape: RoundedRectangleBorder(
                            borderRadius: BorderRadius.circular(12)),
            ...
            ElevatedButton(
              onPressed:
                  _isLoading || _resolvedRecipient == null ? null : _transfer,
              style: ElevatedButton.styleFrom(
                backgroundColor: AppColors.primaryBlue,
